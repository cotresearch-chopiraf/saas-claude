...
| **Actual Cost** | Σ `expenses.amount`, optionally linked to a `budgetItems` row via `budgetItemId` | Incurred cost | Expense CRUD (`routes/budget.ts`, member-accessible, audited as of Phase 1.1); also `POST /payroll-periods/:id/post` (MIDAD Phase A5, owner-only via `payroll.post`), which creates ordinary `expenses` rows from approved Labor Allocations — `labor_cost_postings` is a provenance/bridge table only, never a second Actual Cost source (see `schema.ts`'s comment on that table) |
