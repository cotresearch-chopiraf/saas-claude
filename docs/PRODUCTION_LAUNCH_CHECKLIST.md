...
- [ ] **CORS production origin confirmed — EXTERNAL CONFIRMATION REQUIRED.** The mechanism now exists: `server/src/lib/corsOrigins.ts` + `CORS_ORIGIN` env var restricts allowed origins when set (comma-separated), live-verified both for an allowed origin (`Access-Control-Allow-Origin` reflects it) and a disallowed one (header correctly absent). Production now fails **closed**, not open: `buildCorsOptions()` throws `CorsConfigError` when `NODE_ENV=production` and `CORS_ORIGIN` is unset, and `index.ts` refuses to start the server rather than falling back to permissive (`*`) CORS — dev/test are unaffected (`CORS_ORIGIN` stays optional there). **Action required at deploy time:** set `CORS_ORIGIN` to the actual production frontend URL(s); until it's set, the production server will not start at all.
...
- [ ] **Restore test completed — EXTERNAL CONFIRMATION REQUIRED.** `docs/BACKUP_STRATEGY.md` now documents the full restore procedure (9 explicit steps, stop-writes through verify-business-data), RPO/RTO targets, an isolated restore-verification drill diagram, and a disaster-recovery scenario table (database failure, accidental deletion, corrupted migration, storage failure, deployment rollback, credential compromise, host failure). A local demonstration of the drill's mechanics (`pg_dump`/`pg_restore`/row-count verification/connectivity check) was performed against this environment's disposable local test database on 2026-09-08 and succeeded (see that document's "Restore verification drill" section for the exact result) — this proves the documented commands are correct, but it used no production credentials, data, or infrastructure, and is explicitly not a substitute for the mandatory production drill. **The actual production backup and restore drill remain UNVERIFIED** — no production hosting provider is chosen yet, so there is nothing to run it against. **Date of last production restore test: none.**
