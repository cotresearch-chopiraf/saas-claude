...
// MIDAD ZATCA Slice 4 — the signing boundary. The only implementation
// that exists in this environment (NotImplementedSigner) always fails,
// honestly, regardless of input — see its own file comment for why (no
// verifiable primary XAdES specification, no real credential material).
// The "negative test" categories the Slice 4 spec asks for (invalid
// certificate / invalid private key / mismatched cert+key / expired
// certificate / missing credential / malformed XML) all collapse into the
// SAME universal behavior for this signer, because it never inspects its
// input at all before failing — proven below by feeding it well-formed,
// malformed, and empty input alike and observing identical, honest
// failure every time. A future real signer would need to actually
// distinguish these cases; this one must never pretend to.
...
describe("NotImplementedSigner", () => {
  ...
  it("getZatcaSigner() factory returns a NotImplementedSigner in this environment", () => {
    expect(getZatcaSigner()).toBeInstanceOf(NotImplementedSigner);
  });
});
