...
const createSchema = z.object({
  ...
  originalValue: z.coerce.number().finite().nonnegative(),
  revisedValue: z.coerce.number().finite().nonnegative().optional(),
  currency: z.string().min(1).default("SAR"),
  advancePercent: z.coerce.number().finite().min(0).max(100).optional(),
  retentionPercent: z.coerce.number().finite().min(0).max(100).optional(),
